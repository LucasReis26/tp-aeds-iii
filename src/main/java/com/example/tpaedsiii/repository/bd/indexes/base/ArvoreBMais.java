...
public class ArvoreBMais<T extends RegistroArvoreBMais<T>> {
    ...
    public List<T> readRange(T elemInicio, T elemFim) throws Exception {
        long raiz;
        arquivo.seek(0);
        raiz = arquivo.readLong();

        if (raiz != -1) {
            long primeiraFolha = encontrarFolhaParaLeitura(elemInicio, raiz);
            return lerSequencialmente(primeiraFolha, elemInicio, elemFim);
        } else {
            return new ArrayList<>(); // Árvore vazia
        }
    }


    private long encontrarFolhaParaLeitura(T elem, long pagina) throws Exception {
        ...
        return encontrarFolhaParaLeitura(elem, pa.filhos.get(i));
    }

      private ArrayList<T> lerSequencialmente(long paginaAtual, T elemInicio, T elemFim) throws Exception {
        ...
        while (continuar && paginaAtual != -1) {
            ...
            for (int i = 0; i < pa.elementos.size(); i++) {
                ...
                if (elementoAtual.compareTo(elemFim) > 0) {
                    continuar = false;
                    break; 
                }
                
                if (elementoAtual.compareTo(elemInicio) >= 0) {
                    resultados.add(elementoAtual);
                }
            }
       ...
       public boolean update(T elem) throws Exception {
        boolean deleted = this.delete(elem);
        if (!deleted) {   
            return false;
        }

        boolean created = this.create(elem);
